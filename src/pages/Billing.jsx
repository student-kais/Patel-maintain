...
import { Plus, Search, Trash2, FileText, Printer, CheckCircle, Clock, Calendar } from 'lucide-react';
...
const Billing = () => {
    ...
    const filteredInvoices = invoices.filter(inv => {
        const matchesSearch = inv.customerName.toLowerCase().includes(invoiceSearchTerm.toLowerCase()) ||
            inv.id.toLowerCase().includes(invoiceSearchTerm.toLowerCase()) ||
            (inv.vehicleNumber && inv.vehicleNumber.toLowerCase().includes(invoiceSearchTerm.toLowerCase())) ||
    ...
    const handleCreateInvoice = () => {
        ...
        addInvoice(cart, customerName, serviceCharge || 0, vehicleNumber, customerMobile);
    ...
    const downloadPDF = (invoice) => {
        ...
        const shopSubline = shopDetails?.subline || 'Auto Rickshaw & Three-Wheeler Specialist';
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 animate-fade-in">
                    ...
                    <div className="glass-panel p-6 h-full">
                        ...
                        <div className="overflow-y-auto max-h-[400px]">
                            {filteredProducts.map(product => (
                                <div key={product.id} className="flex justify-between items-center p-3 border-b border-white/10 hover:bg-white/5 rounded">
                                    <div>
                                        <p className="font-medium text-white">{product.name}</p>
                                        <p className="text-xs text-dim">Stock: {product.stock} | ₹{product.price}</p>
                                    </div>
                                    <button
                    ...
                    <div className="glass-panel p-6 h-full flex flex-col">
                        ...
                        <div className="mb-4 grid grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="text-dim text-sm block mb-1">Labor / Service Charge (₹)</label>
                                <input
                                    type="number"
                                    className="input-glass"
                                    placeholder="0"
                ...
                <div className="space-y-6 animate-fade-in">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="glass-panel p-4 flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-dim text-xs uppercase tracking-wider font-semibold">Today's Revenue</p>
                                <h3 className="text-2xl font-bold text-white">₹{stats.today.toLocaleString()}</h3>
                            </div>
                        </div>
                        <div className="glass-panel p-4 flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-dim text-xs uppercase tracking-wider font-semibold">Weekly Revenue</p>
                                <h3 className="text-2xl font-bold text-white">₹{stats.week.toLocaleString()}</h3>
                            </div>
                        </div>
                        <div className="glass-panel p-4 flex items-center gap-4">
                            ...
                            <div>
                                <p className="text-dim text-xs uppercase tracking-wider font-semibold">Monthly Revenue</p>
                                <h3 className="text-2xl font-bold text-white">₹{stats.month.toLocaleString()}</h3>
                            </div>
                        </div>
                    </div>
                            ...
                            <table className="w-full">
                                ...
                                <tbody>
                                    {filteredInvoices.length === 0 ? (
                                        <tr><td colSpan="8" className="text-center p-8 text-dim">No invoices found.</td></tr>
                                        ...
                                        <tr key={inv.id} className="border-b border-white/5 hover:bg-white/5 transition-colors">
                                            ...
                                            <td className="p-4 text-dim text-sm">{inv.customerMobile || '-'}</td>
                                            <td className="p-4 text-dim text-sm">{inv.vehicleNumber || '-'}</td>
                                            <td className="p-4 text-green-400 font-bold">₹{inv.total.toFixed(2)}</td>
                                            <td className="p-4"><span className="badge badge-success text-xs">Paid</span></td>
                                            <td className="p-4">
                                                <button
